...
export CURRENT_VERSION:=$(shell cat ${ROOTDIR}/wpm_reader/_meta.py  | sed -En "s/^__version__\s*=\s*[ubrf]*['\"]([^'\"]+)['\"].*$$/\1/p")
export CURRENT_USER:=$(shell id -u ${USER}):$(shell id -g ${USER})

# Docker configuration
export IMAGE_NAME:=$(or $(IMAGE_NAME), wpm_reader)
...
uninstall: ## uninstall the package
	pip uninstall -y $(PIP_ARGS) wpm_reader || true
...
test: ## run tests with pytest and calculate coverage
	py.test --cov wpm_reader --cov-report term --cov-fail-under 80 \
...
security: ## check source code for vulnerabilities
	@[ "${REPORT_FORMAT}" ] && ( mkdir -p docs/_build/security && bandit -v -r -f ${REPORT_FORMAT} -o docs/_build/security/index.html wpm_reader &> /dev/null ) || true
	bandit -v -r wpm_reader
...
docker-run: ## run the specified command on docker (defaults to /bin/bash)
	docker-compose -f docker/dev/docker-compose.yml run --rm -u $(CURRENT_USER) wpm_reader $(or $(COMMAND),  $(MAKE) $(TARGET))

docker-shell: ## drop the user into a shell inside a docker container
	docker-compose -f docker/dev/docker-compose.yml run --rm -u $(CURRENT_USER) wpm_reader
...
docker-package: ## package an image as a tar archive
	...
	docker save -o ${ROOTDIR}/dist/wpm_reader-$(IMAGE_VERSION).tar $(IMAGE_NAME):$(IMAGE_VERSION)
...
ci-docs: ## generate docs on CI (Github)
	tox -e wpm_reader -- $(MAKE) docs

ci-version-set: ## set the version to the specified VERSION on CI (Github)
	tox -e wpm_reader -- $(MAKE) version-set VERSION="${VERSION}"
...
ci-dist-dev: ## build the wheel package (for development, non cythonized) on CI (Github)
	tox -e wpm_reader -- $(MAKE) dist-dev

ci-publish: ## publish built packages to the appropriate repository on CI (Github)
	tox -e wpm_reader -- $(MAKE) publish ## publish the packages generated with dist or dist-dev on CI (Github)
...
clean-build: ## remove build artifacts
	...
	find wpm_reader -name '*.c' -exec rm -f {} +
